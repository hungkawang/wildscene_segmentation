...
class EncoderBottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, base_width=64):
        ...
        self.d_sample = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False),
            nn.BatchNorm2d(out_channels),
        )

        width = int(out_channels * (base_width / 64))   # Width of intermediate variables for residual link
...
class DecoderBottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.u_sample = nn.Upsample(scale_factor=scale_factor, mode='bilinear', align_corners=True)
    ...
    def forward(self, x, x_concat=None):
        ...
        if x_concat is not None:
            x = torch.cat([x_concat, x], dim=1)     # (batch_size, channels, H, W)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.norm1(x)
        x1 = self.relu(x)   # save to put them together after Transposed Conv
